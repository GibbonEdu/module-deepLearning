...
$URL = $session->get('absoluteURL').'/index.php?q=/modules/Deep Learning/viewMyDL.php&deepLearningEventID='.$deepLearningEventID;
...
if (isActionAccessible($guid, $connection2, '/modules/Deep Learning/view_experience_signUp.php') == false) {
    ...
    if (empty($choices) || empty($gibbonPersonID) || empty($deepLearningEventID)) {
        $URL .= '&return=error1';
        header("Location: {$URL}");
        exit;
    }
    ...
    $signUps = $signUpGateway->selectSignUpsByPerson($deepLearningEventID, $gibbonPersonID)->fetchGroupedUnique();
    ...
    $signUpGateway->deleteSignUpsNotInList($deepLearningEventID, $gibbonPersonID, $choiceIDs);
